...
- **Lead/control-plane goal mode** — `/goal <condition>` launches a dedicated worker, independently evaluates that worker's bounded evidence, and continues the worker on `not_met`. The initiating conversation stays clear for `/goal`, `/goal steer ...`, `/goal pause`, `/goal resume`, and `/goal clear`. Productive goals have no hard turn cap by default; the no-progress detector and four-hour wall-clock backstop bound the loop.
...
- **Automatic token economy** — provider-reported input, output, reasoning, cache-read, cache-write, and cost are persisted. A goal's `softTokens` is a recurring compaction interval: each crossed interval compacts asynchronously, shows a `compacting` state, and releases the durable evaluator continuation exactly once after OpenCode returns idle. `maxTokens` is an optional lifetime-spend backstop; workflow-run budgets retain their existing semantics. Unknown usage remains explicitly unknown.
...
- **Non-blocking TUI startup** — the Solid-based dashboard and activity badge load just after OpenCode's first paint, so their module initialization does not delay the initial prompt.
...
Use `/goal all tests pass and the built artifact loads` to launch a dedicated autonomous worker. `/goal` reports its worker, completed goal turns, elapsed time, observed usage, evaluator, compaction state, and last verdict. `/goal steer run the browser test`, `/goal pause`, `/goal resume`, and `/goal clear` control it without moving implementation traces into the lead conversation. The worker inherits the lead's selected model and agent when OpenCode reports them. A continuation pending behind compaction is durable across hot reload and is sent only after the worker is idle.

Goal workers default to `noProgressLimit: 3`, no hard `maxTurns`, and a four-hour `maxDurationMs` backstop. A **goal turn** is one completed worker-idle → independent-evaluator cycle; it is not a raw model stream step. The measurable progress proxy is fresh assistant tool activity observed since the previous evaluation (message IDs make this exact across transcript compaction; providers without IDs fall back to the latest assistant response). Three consecutive `not_met` goal turns without that proxy pause the goal for operator review. Set `maxTurns` explicitly only when an absolute evaluator-cycle ceiling is desired.

Goal workers compact every `goalSoftTokens` provider-reported lifetime tokens
(default 180000). After a compaction at total `T`, the next boundary is
`T + goalSoftTokens`; a small subsequent turn does not immediately compact
again. `goalMaxTokens` is unset by default and, when explicitly supplied, is a
lifetime-spend ceiling independent of compaction. Existing persisted goals that
already carry a `maxTokens` value keep that ceiling.
