...
- **Separate TUI target** — `opencode-plugin-orch/tui` adds a responsive multi-row prompt-side activity badge for the current session's goal worker and active workflows. It drops optional detail and collapses workflow rows on narrow terminals; token/cost detail stays in tools and the read-only dashboard instead of crowding the statusline. The home/start view never adopts goals from other sessions.
